...
Incoming.prototype._extractOrderInfo = function (version, isCompletedOrder) {
    var order = {};
    // read contract fields
    var contract = {};
    ...
    if (version >= 25 && !isCompletedOrder) {
...
Incoming.prototype._OPEN_ORDER = function () {
  ...
  const [contract, order, orderState] = this._extractOrderInfo(version, false)
  order.orderId = orderId

  this._emit('openOrder', orderId, contract, order, orderState);
};
...
Incoming.prototype._COMPLETED_ORDER = function () {
  const [contract, order, orderState] = this._extractOrderInfo(9999, true)
  this._emit('completedOrder', contract, order, orderState);
};
...
Incoming.prototype._NEWS_PROVIDERS = function () {
  ...
  this._emit('newsProviders', providers);
};
...
Incoming.prototype._HISTORICAL_NEWS_END = function () {
  var version = this.dequeueInt();
  var reqId = this.dequeueInt();
  var hasMore = this.dequeueBool();

  this._emit('historicalNewsEnd', reqId, hasMore);
};

Incoming.prototype._NEWS_ARTICLE = function () {
  var reqId = this.dequeueInt();
  var articleType = this.dequeueInt();
  var articleText = this.dequeue();

  this._emit('newsArticle', reqId, articleType, articleText);
};

Incoming.prototype._MKT_DEPTH_EXCHANGES = function () {
  const descriptions = [];
  const nDescriptions = this.dequeueInt();

  for (let i = 0; i < nDescriptions; i++)
  {
    if (this._controller._serverVersion >= 120) {
			descriptions.push({
				exchange: this.dequeue(),
				secType: this.dequeue(),
				listingExch: this.dequeue(),
				serviceDataType: this.dequeue(),
				aggGroup: this.dequeueInt() || 0
			})
		} else {
      descriptions.push({
        exchange: this.dequeue(),
...
Incoming.prototype._SYMBOL_SAMPLES = function () {
  var reqId = this.dequeueInt();
  var nContractDescriptions = this.dequeueInt();
  ...
  for (let i = 0; i < nContractDescriptions; i++) {
    ...
    const derivativeSecTypes = [];

    for (let j = 0; j < nDerivativeSecTypes; j++)
    {
        derivativeSecTypes.push(this.dequeue());
    }

    contractDescriptions.push({ contract, derivativeSecTypes })
